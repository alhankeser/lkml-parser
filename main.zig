...
pub fn main() !void {
    ...
    var isInField = false;
    var isExpectingFieldClosingCurlyBraces = false;
    ...
    while (it.next()) |word| {
        ...
        if (isInObject and isValidKey(1, word)) {
            // close field
            if (isExpectingFieldClosingCurlyBraces) {
                isExpectingFieldClosingCurlyBraces = false;
                print("{s}", .{"\n}"});
            }
            print("  \"{s}\" {{\n", .{word});
        ...
        if (isPreviousFieldKey) {
            ...
            isExpectingFieldClosingCurlyBraces = true;
            isInField = true;
            continue;
        }
